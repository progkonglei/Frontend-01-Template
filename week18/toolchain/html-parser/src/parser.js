...
function tagName(c){
    if(c.match(/^[\t|\n|\f ]$/)){
        ...
        if(currentToken.attr){
            currentAttrs.push(currentToken.attr)
        }
        currentToken.attr = currentAttrs
        emit(currentToken) 
        return data
    }else if(c == EOF){
...
function afterQuotedAttributeValue(c){
    if(c.match(/^[\t|\n|\f ]$/)){
        ...
        //currentToken[currentToken.attr.name] = currentToken.attr.value
        //Switch to the data state. Emit the current tag token.
        if(currentToken.attr){
            currentAttrs.push(currentToken.attr)
        }
...
function emit(token){ //提交我们生成的token
    ...
    if(token.type == 'startTag'){
        ...
        for (let prop in token) {
            if(prop!=="type" && prop!=='tagName'){
                if(Array.isArray(token[prop])){
                    element.attributes.push(...token[prop].map(attrItem=>({
                        name:prop,
                        value:attrItem
                    })))
                }
        ...
        //console.log('get token content !!!!',token.content,currentTextNode.content)
    }
}

export function parseHTML(html){
    let state = data;
    stack =[{type:'document',children:[]} ]//reset stack whenever before parse
    for (let c of html) {
        state = state(c)
