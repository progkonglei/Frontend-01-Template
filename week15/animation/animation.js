export class Timeline{
    constructor(){
        ...
        this.tick = ()=>{//每一帧执行的函数用tick来实现
            ...
            let animations = this.animations.filter(animation=>!animation.finished)//是循环正在进行的animation
            //for循环换所有的animation
            for (const animation of this.animations) {
                let {object,property,template,start,end,delay,timingFunction,duration,addTime} = animation
            ...
            if(animations.length){//没有正在进行的animation时候，不再requestAnimationFrame
    ...
    add(animation,addTime){//加animation的时候需要知道这个animation是从多长时间之后开始执行，还是从0（马上）开始
        ...
        if(this.state === 'playing'){
            animation.addTime = addTime !== void 0 ? addTime : Date.now() - this.startTime; //addTime是个时间段(也与progress动画的进程有关的)，要么是从加入进来立即执行，要么是传入个addTime，注意与deley的区分，deley是在每个animation自己身上的，自己决定
        }else{
